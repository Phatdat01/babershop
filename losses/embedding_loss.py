...
class EmbeddingLossBuilder(torch.nn.Module):
    ...
    def _loss_l2(self, gen_im, ref_im):
        if gen_im.shape != ref_im.shape:
            ref_im = torch.nn.functional.interpolate(ref_im, size=gen_im.shape[-2:], mode='bilinear', align_corners=False)
